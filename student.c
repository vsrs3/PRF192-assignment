...
#include <ctype.h>        // Thư viện kiểm tra kiểu ký tự.
#include <limits.h>       // Thư viện khai báo các giá trị giới hạn.
...
void addStudent(const char *filename) {
    ...
    scanf("%d", &s.id); // Nhập mã số sinh viên.
...
/*
 * Hàm loadStudents:
 * Đọc và phân tích từng dòng của file để lấy thông tin sinh viên.
...
/*
 * Hàm saveStudents:
 * Ghi toàn bộ danh sách sinh viên ra file theo định dạng quy định.
 * Trả về 1 nếu ghi thành công, 0 nếu ghi lỗi.
 */
int saveStudents(const char *filename, Student students[], int count) {
...
/*
 * Hàm toLowerStr:
 * Chuyển toàn bộ ký tự trong chuỗi sang chữ thường.
 * Dùng để chuẩn hóa khi so sánh chuỗi.
 */
void toLowerStr(char *str) {
